...
export default function Signup() {
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    try {
      // 1. Send data to the backend passenger signup route
      await axiosInstance.post("/auth/signup", formData);
      
      // 2. Because it's a passenger, redirect directly to the home page!
      navigate("/"); 
    } catch (error) {
    ...
    <div className="flex flex-col items-center justify-center min-h-screen bg-gray-900 text-white p-4">
      ...
      <div className="w-full max-w-md bg-gray-800 p-8 rounded-lg shadow-lg">
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <button type="submit" className="w-full bg-blue-600 hover:bg-blue-700 text-white p-3 rounded font-bold mt-4">
            Sign Up & Ride
          </button>
        </form>
